    ...
    <header className="fixed left-0 right-0 top-0 z-50 border-b border-white/10 bg-ink/88 text-white shadow-strong backdrop-blur-xl">
      <div className="shell flex h-[72px] items-center justify-between gap-4 py-3">
        <Link href="/" onClick={() => setOpen(false)} className="flex min-w-0 items-center gap-3">
          ...
          <span className="truncate font-bold">Lubis Mental Awareness</span>
        </Link>
        ...
        <div className="flex items-center gap-2">
          <a
            href={whatsappHref("Hello Lubis Mental Awareness, I want to get involved.")}
            target="_blank"
            rel="noreferrer"
      ...
      {open && (
        <nav className="shell grid gap-2 pb-4 md:hidden">
          ...
          <Link href="/programs" onClick={() => setOpen(false)} className="rounded-full border border-white/12 px-4 py-3 text-sm font-bold text-white/90">
            Programs
          </Link>
          <Link href="/events" onClick={() => setOpen(false)} className="rounded-full border border-white/12 px-4 py-3 text-sm font-bold text-white/90">
            Events
          </Link>
          <Link href="/#participate" onClick={() => setOpen(false)} className="rounded-full border border-white/12 px-4 py-3 text-sm font-bold text-white/90">
            Participate
          </Link>
        </nav>
      )}
